...
ggplot(merged_data_intercept, aes(x = actual_intercept_2_scaled, y = actual_intercept_1_scaled, color = cesd_category)) +
  ...
  geom_smooth(aes(group = cesd_category), method = "lm", se = TRUE, size = 1, alpha = 0.05) +  # Modellvorhersagen mit Konfidenzintervallen
  labs(
...
ggsave("moderation_plot_ohneIntervall.png", 
...
#plot cut off 16
merged_data_slope$cesd_category <- as.factor(merged_data_slope$cesd_category)
...
ggsave("moderation_plot_slope_ohneIntervall.png", 
